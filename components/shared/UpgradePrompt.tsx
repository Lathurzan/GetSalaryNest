...
export default function UpgradePrompt({
  ...
  const goToCheckout = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        setBusy(false);
        // not logged in → send to login, then back to billing
        if (res.status === 401) {
          window.location.href = "/login?callbackUrl=/settings/billing";
          return;
        }
        return setError(data.error ?? "Could not start checkout");
      }
        ...
        <div className="p-6 pb-[calc(env(safe-area-inset-bottom)+1.5rem)] sm:p-7 sm:pb-7">
          ...
          <div className="mb-5 flex items-start justify-between gap-4">
            <div className="flex items-start gap-3">
              <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br from-teal-500/20 to-emerald-500/10 ring-1 ring-teal-500/25">
                <Sparkles size={19} strokeWidth={2} className="text-teal-600 dark:text-teal-400" />
              </div>
              <div>
          ...
          >
            ...
            <span className="relative flex items-center gap-2">
              ...
              {busy ? "Opening checkout…" : "See plans"}
            </span>
          </button>
